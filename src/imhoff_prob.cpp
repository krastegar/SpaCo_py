...
extern "C" {
  ...
  { // See Imhof (1961), p.423
    ...
    double sum = 0.0;
    m = lambdalen[0];
    for (i = 1; i <= m; i = i + 1) sum = sum + h[i - 1] * std::atan(lambda[i - 1] * u[0]) + delta2[i - 1] * lambda[i - 1] * u[0] / (1.0 + pow(lambda[i - 1] * u[0], 2.0));
  ...
  { // See Imhof (1961), p.423
    ...
    double prod = 1.0;
    m = lambdalen[0];
    for (i = 1; i <= m; i = i + 1)  prod = prod * pow(1.0 + pow(lambda[i - 1] * u[0], 2.0), 0.25 * h[i - 1]) * std::exp(0.5 * delta2[i - 1] * pow(lambda[i - 1] * u[0], 2.0) / (1.0 + pow(lambda[i - 1] * u[0], 2.0)));
    return(prod);
  }
...
// Wrapper functions for pybind11
double py_probQsupx(double x, std::vector<double> lambda, std::vector<double> h, std::vector<double> delta2, double epsabs, double epsrel, int limit) {
    int lambdalen = lambda.size();
    double Qx[1];
    probQsupx(&x, lambda.data(), &lambdalen, h.data(), delta2.data(), Qx, &epsabs, &epsrel, &limit);
    return Qx[0];
  }
  
  PYBIND11_MODULE(imhoff_prob, m) {
    m.doc() = "Imhof (1961) algorithm module";
    m.def("imhoffunc", [](std::vector<double> u, std::vector<double> lambda, std::vector<double> h, std::vector<double> x, std::vector<double> delta2) {
      int lambdalen = lambda.size();
      return imhoffunc(u.data(), lambda.data(), &lambdalen, h.data(), x.data(), delta2.data());
    }, "Compute the Imhof function",
      py::arg("u"), py::arg("lambda"), py::arg("h"), py::arg("x"), py::arg("delta2"));
    m.def("probQsupx", &py_probQsupx, "Calculate the survival function of quadratic forms in normal variables using Imhof's method",
          py::arg("x"), py::arg("lambda"), py::arg("h"), py::arg("delta2"), py::arg("epsabs") = 1e-6, py::arg("epsrel") = 1e-6, py::arg("limit") = 10000);
  }